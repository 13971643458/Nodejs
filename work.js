    ...
    formatXlsxFile() {
        ...
        for (let i = 1; i < this.inputList.length; i++) {
            ...
            if (!curDate) {
                ...
                // console.log("拿当月第一天出maxDate="+maxDate);
            }
            if (this.isSameDay(curDate, date)) {
                if (action == '出') {
                    maxDate = date;

                    // if (this.isAddedWork(maxDate)){
                ...
                if (this.isAddedWork(maxDate)) {
                    ...
                    this.outputList.push([row[1], row[2], minDate.toLocaleString(), '进', productName, '', '', '']);
                    this.outputList.push([row[1], row[2], maxDate.toLocaleString(), '出', productName, '25元', maxDate.getHours()>=21?'80元':'', '']);
                     console.log("拿当月出的数据maxDate=" + maxDate);

                    if (maxDate.getHours()>=21){
                        che.push(maxDate);
                    }
                }
                curDate = date;
        ...
        count = (count - 1) / 2;
        let cheF=che.length*80;
        let fan=25 * count;
        let tongj=cheF+fan;
        let outRow = ['', '', '', '', '合计', tongj+'元', '', ''];
